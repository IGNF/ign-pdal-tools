...
INPUT_LIGNES = os.path.join(DATA_LIGNES_PATH, "Lignes_3d_0292_6833.geojson")
...
@pytest.mark.parametrize(
    "input_file, input_points, epsg, expected_nb_points, spacing",
    [
        #(INPUT_PCD, INPUT_POINTS, None, 678, 0),  # should add only points within tile extent
        # (INPUT_PCD_CROPPED, INPUT_POINTS_FOR_CROPPED_PCD, None, 186, 0),
        # (
        #     INPUT_PCD_CROPPED,
        #     INPUT_POINTS,
        #     None,
        ...
        (INPUT_PCD, INPUT_LIGNES, "EPSG:2154", 656, 0.25),  # should add only lignes within tile extend
    ],
)
def test_add_points_from_geojson_to_las(input_file, input_points, epsg, expected_nb_points, spacing):
    # Ensure the output file doesn't exist before the test
    if Path(OUTPUT_FILE).exists():
        os.remove(OUTPUT_FILE)

    add_points_in_pointcloud.add_points_from_geojson_to_las(input_points, input_file, OUTPUT_FILE, 68, epsg, 1000, spacing)
    assert Path(OUTPUT_FILE).exists()  # check output exists
    point_count = compute_count_one_file(OUTPUT_FILE)["68"]
    assert point_count == expected_nb_points  # Add all points from geojson
